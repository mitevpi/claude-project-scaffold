...
when releases come from a separate branch. For a solo repo where you own `main`, use
`CLAUDE-light.md` instead. The two files differ only in the Git section's branch model.
...
- **The base branch is `{{feature-dev}}`, not `{{main}}`.** `{{main}}` belongs to the repo
  owner for releases. Never check it out, commit to it, merge into it, or delete it. Never
  delete `{{feature-dev}}` either. The owner promotes `{{feature-dev}}` to `{{main}}` by
  hand. Do not open or suggest a PR that targets `{{main}}`.
- **Cut one short-lived branch per plan or task from `{{feature-dev}}`.** Never reuse a
  branch for unrelated work. You may create these without permission. Every other branch
  operation needs permission.
- **Land a branch with `.claude/scripts/land-branch.sh <branch> {{feature-dev}}`.** It
  ...
  When `{{feature-dev}}` tracks a remote, run `git pull --ff-only` in its checkout first.
